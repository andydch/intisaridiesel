                ...
                <div class="col-xl-12 mx-auto">
                                    ...
                                    <div class="row">
                                        <div class="col-xl-6">
                                            ...
                                            <div class="row mb-3">
                                                ...
                                                <label for="" class="col-sm-9 col-form-label part-id">{{ $qRo->receipt_order->invoice_no }}</label>
                                            </div>
                                            <div class="row mb-3">
                                                <label for="" class="col-sm-3 col-form-label">Ship By</label>
                                                {{-- <label for="" class="col-sm-9 col-form-label">{{ $qRo->courier->name }}</label> --}}
                                        ...
                                        <div class="col-xl-6">
                                            ...
                                            <div class="row mb-3">
                                                ...
                                                <label for="" class="col-sm-9 col-form-label part-id">{{ $qRo->receipt_order->receipt_no }}</label>
                                            </div>
                                            <div class="row mb-3">
                                    ...
                                    <div class="row">
                                        <div class="col-xl-6">
                                            <div class="row mb-3">
                                                ...
                                                <div class="col-sm-9">
                                                    @if(is_null($qRo->approved_by) && is_null($qRo->canceled_by))
                                                        <label for="active" class="col-sm-9 col-form-label">{{ 'Waiting for Approval' }}</label>
                                                    @endif
                                                    @if(!is_null($qRo->approved_by))
                                                        <label for="active" class="col-sm-9 col-form-label">
                                                            {{ 'Approved at '.
                                                                date_format(date_add(date_create($qRo->approved_at), 
                                                                date_interval_create_from_date_string(ENV("WAKTU_ID")." hours")), 'd-M-Y H:i:s').
                                                                ' by '.$qRo->approvedBy->name }}
                                                        </label>
                                                        {{-- <label for="active" class="col-sm-9 col-form-label">{{ 'Approved by '.$qRo->approvedBy->name.' at '.date_format(date_create($qRo->approved_at), 'd M Y H:i:s') }}</label> --}}
                                                    @endif
                                                    @if(!is_null($qRo->canceled_by))
                                                        <label for="active" class="col-sm-9 col-form-label">
                                                            {{ 'Rejected at '.
                                                                date_format(date_add(date_create($qRo->canceled_at), 
                                                                date_interval_create_from_date_string(ENV("WAKTU_ID")." hours")), 'd-M-Y H:i:s').
                                                                $qRo->canceledBy->name }}
                                                        </label>
                                                        {{-- <label for="active" class="col-sm-9 col-form-label">{{ 'Rejected by '.$qRo->canceledBy->name.' at '.date_format(date_create($qRo->canceled_at), 'd M Y H:i:s') }}</label> --}}
                                                    @endif
                                                </div>
                                            </div>
                                            <div class="row mb-3">
                        ...
                        <div class="card-body">
                            ...
                            <table class="table table-bordered mb-0">
                                ...
                                <tbody id="new-row">
                                    ...
                                    @foreach ($qRoPart as $rowPart)
                                        <tr id="row{{ $lastIdx }}">
                                            ...
                                            <th scope="row" style="text-align:right;">
                                                ...
                                                {{-- <input type="hidden" name="po_mo_no{{ $lastIdx }}" id="po_mo_no{{ $lastIdx }}" value="{{ $query->po_mo_no }}"> --}}
                                            ...
                                            <td><label for="" name="part_name{{ $lastIdx }}" id="part_name{{ $lastIdx }}" class="col-form-label">{{ $query->part_name }}</label></td>
                                    ...
                                    <tr>
                                        <td colspan="6" style="text-align: right;">
                                            <label for="" name="vat_lbl" id="vat_lbl" class="col-form-label">VAT {{ number_format($qRo->vat_val,0,'.',',') }}%</label>
                                        </td>
                                        <td style="text-align: right;">
                    ...
                    <div class="card" style="margin-top: 15px;">
                        <div class="card-body">
                            <div class="row mb-3">
                                <div class="col-sm-12">
                                    <input @if (!is_null($qRo->approved_by) || !is_null($qRo->canceled_by)){{ 'disabled' }}@endif type="button" id="save-approval-status-btn"
                                        class="btn btn-primary px-5" value="Approve">
