...
<div class="page-wrapper">
    <div class="page-content">
        ...
        <!--end breadcrumb-->
        <div class="row">
            <form id="submit-form" action="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/'.$folder.'/'.$memos->id) }}" method="POST" enctype="application/x-www-form-urlencoded">
                @csrf
                @method('PUT')
                ...
                <div class="col-xl-12 mx-auto">
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="border p-4 rounded">
                                <div class="row mb-3">
                                    <label for="" class="col-sm-3 col-form-label">Memo No</label>
                                    <input type="hidden" name="memo_no_tmp" class="@error('memo_no_tmp') is-invalid @enderror">
                                    <label for="" class="col-sm-9 col-form-label part-id">{{ $memos->memo_no }}</label>
                                    @error('memo_no_tmp')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="row mb-3">
                                    <label for="" class="col-sm-3 col-form-label">Memo Date</label>
                                    <label for="" class="col-sm-4 col-form-label">{{ date_format(date_create($memos->memo_date), 'd/m/Y') }}</label>
                                </div>
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-select single-select @error('supplier_id') is-invalid @enderror" id="supplier_id" name="supplier_id">
                                            ...
                                            @php
                                                $supplierId = (old('supplier_id')?old('supplier_id'):$memos->supplier_id);
                                            @endphp
                                            @foreach ($suppliers as $p)
                                                <option @if ($supplierId==$p->id) {{ 'selected' }} @endif value="{{ $p->id }}">{{ $p->name }}</option>
                                            @endforeach
                                        </select>
                                        @error('supplier_id')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-select @error('supplier_pic') is-invalid @enderror"
                                            ...
                                            <option value="#">Choose...</option>
                                            @php
                                                $supplierPic = (old('supplier_pic')?old('supplier_pic'):$memos->pic_idx);
                                            @endphp
                                    ...
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-sm-9">
                                            <select class="form-select single-select @error('branch_id') is-invalid @enderror"
                                                ...
                                                <option value="">Choose...</option>
                                                @php
                                                    $branch_id = old('branch_id')?old('branch_id'):$memos->branch_id;
                                                @endphp
                                                @foreach ($branches as $p)
                                                    <option @if($branch_id==$p->id){{ 'selected' }}@endif value="{{ $p->id }}">{{ $p->name.$branch_id }}</option>
                                                @endforeach
                                            </select>
                                            @error('branch_id')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        ...
                                        @if (old('vat') == 'on')
                                            ...
                                            @endphp
                                        @endif
                                        <input class="form-check-input" type="checkbox" id="vat" name="vat" aria-label="Active" @if ($vat=='Y' ){{ 'checked' }}@endif>
                                    </div>
                                </div>
                                <div class="row mb-3">
                                    <label for="remark_txt" class="col-sm-3 col-form-label">Remark</label>
                                    <div class="col-sm-9">
                                        <textarea name="remark_txt" id="remark_txt" rows="3" class="form-control @error('remark_txt') is-invalid @enderror" style="width:100%;">@if (old('remark_txt')){{ old('remark_txt') }}@else{{ $memos->remark }}@endif</textarea>
                                        @error('remark_txt')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-8">
                                        <input readonly type="text" name="total_price" id="total_price" class="form-control @error('total_price') is-invalid @enderror"
                                            ...
                                            value="@if (old('total_price')){{ old('total_price') }}@else{{ number_format($queryMemoPartSUM->tot,0,'.',',') }}@endif">
                                        @error('total_price')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                                ...
                                                <tr id="row{{ $i }}">
                                                    ...
                                                    @php
                                                        ...
                                                        ->where([
                                                            ...
                                                            'tx_qty_parts.branch_id' => $memos->created_by
                                                        ])
                                                        ->first();
                                                    @endphp
                                                    <td>
                                                        <input type="hidden" name="memo_part_id_{{ $i }}" id="memo_part_id_{{ $i }}" value="0">
                                                    ...
                                                    <td>
                                                        <input type="text" onkeyup="totPrice({{ $i }});"
                                                    ...
                                                    <td style="text-align: right;">
                                                        <label id="total-price-{{ $i }}" for="" class="col-form-label">@if (old('price'.$i)){{ number_format(old('qty'.$i)*$price,0,'.',',') }}@else{{ '0' }}@endif</label>
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td style="text-align: right;"><label id="final-cost-{{ $i }}" for="" class="col-form-label">{{ (is_null($partNo)?'':number_format($partNo->final_cost,0,'.',',')) }}</label></td>
                                                    <td style="text-align: right;">
                                    ...
                                    @else
                                        ...
                                        @foreach($memoParts AS $mp)
                                            <tr id="row{{ $i }}">
                                                ...
                                                @endphp
                                                <td>
                                                    <input type="hidden" name="memo_part_id_{{ $i }}" id="memo_part_id_{{ $i }}" value="{{ $mp->id }}">
                                                    <select onchange="dispPartRef_PartNo(this.value, {{ $i }});" class="form-select partsAjax @error('part_id'.$i) is-invalid @enderror"
                                                        id="part_id{{ $i }}" name="part_id{{ $i }}">
                                                        <option value="#">Choose...</option>
                                                        @php
                                                            $partId = old('part_id'.$i) ? old('part_id'.$i) : $mp->part_id;
                                                            $partList = \App\Models\Mst_part::where([
                                                                'id' => $partId,
                                                            ])
                                                            ->get();
                                                ...
                                                <td>
                                                    <input type="text" onkeyup="formatPartPrice({{ $i }});"
                                                        class="form-control @error('price'.$i) is-invalid @enderror"
                                                        id="price{{ $i }}" name="price{{ $i }}" maxlength="25"
                                                        value="{{ ($mp->price==0)?'':number_format($mp->price,0,'.',',') }}" style="text-align: right;" />
                                                    @error('price'.$i)
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                    @enderror
                                                </td>
                                                <td style="text-align: right;"><label id="total-price-{{ $i }}" for="" class="col-form-label">{{ number_format($mp->qty*$mp->price,0,'.',',') }}</label></td>
                                                <td>
                                                    <textarea class="form-control" name="desc_part{{ $i }}" id="desc_part{{ $i }}" rows="3"
                                                ...
                                                @endphp
                                                {{-- <input type="hidden" name="" value="{{ $partNo->purchase_memo_qty }}">
                                                <input type="hidden" name="" value="{{ $partNo->purchase_order_qty }}">
                                                <input type="hidden" name="" value="{{ $partNo->purchase_ro_qty }}">
                                                <input type="hidden" name="" value="{{ $partNo->purchase_ro_qty_no_partial }}"> --}}
                                                <td style="text-align: right;">
                                                    <label id="oo-{{ $i }}" for="" class="col-form-label">{{ number_format($oo,0,'.',',') }}</label>
                                                </td>
                                                <td style="text-align: center;"><input type="checkbox" id="rowCheck{{ $i }}" value="{{ $i }}"></td>
                    ...
                    @if ($memos->active=='Y' && is_null($memos->receipt_order))
                        <hr>
                        <div class="card" style="margin-top: 15px;">
                            <div class="card-body">
                                <div class="row mb-3">
                                    <div class="col-sm-12">
                                        ...
                                        @if (($memos->created_by==Auth::user()->id || $userLogin->is_director=='Y') &&
                                            $memos->active=='Y' && is_null($memos->receipt_order))
...
<script>
    ...
    function dispPartRef(part_id, idx){
        ...
        fd.append('memo_created_by', '{{ $memos->created_by }}');
        $.ajax({
    ...
    $(document).ready(function() {
        ...
        @if ($memos->created_by==Auth::user()->id && $memos->active=='Y' && is_null($memos->receipt_order))
            $("#del-btn").click(function() {
                let msg = 'The following Memo Numbers will be canceled.\n{{ $memos->memo_no }}\nContinue?';
                if(!confirm(msg)){
                    event.preventDefault();
                }else{
                    $("#memoId").val('{{ $memos->id }}');
                    ...
                    $('#submit-form').attr('action', "{{ url('/del_memo') }}");
                    $("#submit-form").submit();
                }
            });
        @endif
        ...
        $("#btn-add-row").click(function() {
            ...
            let vHtml =
                ...
                '<td>'+
                    ...
                    '</select>'+
                '</td>'+
                '<td><input onchange="totPrice('+totalRow+');" type="text" class="form-control" id="qty'+totalRow+'" name="qty'+totalRow+'" '+
                'maxlength="5" style="text-align: right;" value="" /></td>'+
                '<td><input onkeyup="formatPartPrice('+totalRow+');" type="text" class="form-control" id="price'+totalRow+'" name="price'+totalRow+'" '+
                'maxlength="25" style="text-align: right;" value="" /></td>'+
