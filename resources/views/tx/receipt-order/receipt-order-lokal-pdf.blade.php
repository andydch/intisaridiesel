            ...
            <table style="width: 100%;background-color: white;border:0px solid white !important;margin-bottom: 25px;">
                <tr style="border:0px solid white !important;">
                    <td style="width: 50%;text-align: left;border:0px solid white !important;">
                        <span style="font-size: 12px;">FR : {{ (!is_null($receipt_orders->supplier_entity_type)?$receipt_orders->supplier_entity_type->title_ind:'').' '.(!is_null($receipt_orders->supplier)?$receipt_orders->supplier->name:'') }}</span><br/>
                    ...
                    <td style="width: 50%;text-align: left;vertical-align: top;border:0px solid white !important;padding-left: 175px;">
                        {{-- <span style="font-weight: bold;font-size: 13px;">RECEIPT ORDER</span><br/> --}}
                        <span style="font-size: 10px;">{{ $receipt_orders->branch?$receipt_orders->branch->name:'' }}, {{ date_format(date_create($receipt_orders->receipt_date),"d/m/Y") }}</span><br/>
                        {{-- <span style="font-size: 10px;font-weight:bold;">RO : {{ $receipt_orders->receipt_no }}</span> --}}
                    </td>
                </tr>
            </table>
            ...
            <table style="width: 100%;margin-bottom: 25px;">
                <thead>
                    <tr>
                        ...
                        <th>Parts Name</th>
                        <th colspan="2">Qty</th>
                        <th>Price<br/>({{ (!is_null($receipt_orders->currency)?$receipt_orders->currency->string_val:$qCurrency->string_val) }})</th>
                        <th>Total<br/>({{ (!is_null($receipt_orders->currency)?$receipt_orders->currency->string_val:$qCurrency->string_val) }})</th>
                        <th>AVG Cost<br/>({{ (!is_null($receipt_orders->currency)?$receipt_orders->currency->string_val:$qCurrency->string_val) }})</th>
                    </tr>
                </thead>
                <tbody>
                    @php
                    ...
                    <tr>
                        ...
                        <td class="val-num">
                            {{ $receipt_orders->vat_val>0?number_format($tot*$vat/100,0,'.',','):'' }}
                        </td>
                    </tr>
                    <tr>
                        <td colspan="6" style="text-align: right;">
                        ...
                        <td class="val-num">
                            {{ $receipt_orders->vat_val>0?number_format(($tot*$vat/100)+$tot,0,'.',','):number_format($tot,0,'.',',') }}
                        </td>
                    </tr>
                </tbody>
            </table>
