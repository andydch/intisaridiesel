                    ...
                    <table class="table table-striped table-bordered" id="recipt-order" style="width:100%">
                        ...
                        <tbody>
                            ...
                            @foreach ($query as $pV)
                                <tr>
                                    <td>
                                        <a href="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/'.$folder.'/'.urlencode($pV->kwitansi_no)) }}" style="text-decoration: underline;">{{ $pV->kwitansi_no }}</a>
                                    </td>
                                    <td>{{ date_format(date_create($pV->kwitansi_date), 'd/m/Y') }}</td>
                                    <td>{{ !is_null($pV->customer)?$pV->customer->name:'' }}</td>
                                    <td style="text-align: right;">{{ number_format($pV->np_total,0,'.',',') }}</td>
                                    ...
                                    <td>
                                        ...
                                        @endif
                                        @if($pV->active=='Y' && strpos($pV->kwitansi_no,'Draft')==0 && is_null($pV->approved_by) && is_null($pV->canceled_by))
