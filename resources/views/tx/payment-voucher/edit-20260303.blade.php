                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-select single-select @error('tagihan_supplier_id') is-invalid @enderror" id="tagihan_supplier_id" name="tagihan_supplier_id">
                                            ...
                                            @php
                                                ...
                                                // ->when($qPaymentInv->id!=0, function($qX) use($qPaymentInv, $supplierId) {
                                                //     $qX->where(function($q) use($qPaymentInv, $supplierId) {
                                                //         $q->where(function($q1) use($qPaymentInv, $supplierId){
                                                //             $q1->whereNotIn('id', function($q2) use($qPaymentInv, $supplierId) {
                                                //                 $q2->select('tagihan_supplier_id')
                                                //                 ->from('tx_payment_vouchers')
                                                //                 ->whereRaw('id<>'.$qPaymentInv->id)
                                                //                 ->where('supplier_id', $supplierId)
                                                ...
                                                //             });
                                                //         })
                                                //         ->orWhere(function($q1) use($qPaymentInv, $supplierId){
                                                //             $q1->whereIn('id', function($q2) use($qPaymentInv, $supplierId) {
                                                //                 $q2->select('tagihan_supplier_id')
                                                //                 ->from('tx_payment_vouchers')
                                                //                 ->whereRaw('id<>'.$qPaymentInv->id)
                                                //                 ->where('supplier_id', $supplierId)
...
<script>
    ...
    function sumTerbayar(){
        ...
        $('#biaya-lain-lain').text((parseFloat(adminBank)+parseFloat(biayaKirim)+parseFloat(biayaAsuransi)-parseFloat(diskonPembelian)).numberFormat(0,'.',','));
        ...
        if ($('#payment_type_id option:selected').val()=='P'){
            ...
            grandTotal = parseFloat(totNum+vat_num)+parseFloat(adminBank)+parseFloat(biayaKirim)+parseFloat(biayaAsuransi)-parseFloat(diskonPembelian);
            $('#grand-tot-terbayar').text(grandTotal.numberFormat(0,'.',','));
        }else{
            grandTotal = parseFloat(totNum)+parseFloat(adminBank)+parseFloat(biayaKirim)+parseFloat(biayaAsuransi)-parseFloat(diskonPembelian);
            $('#grand-tot-terbayar').text(grandTotal.numberFormat(0,'.',','));
        }
    ...
    $(document).ready(function() {
        ...
        $("#payment_mode_id").change(function() {
            ...
            if($('#payment_mode_id option:selected').val()==1 || $('#payment_mode_id option:selected').val()==3){
                $("#tagihan_supplier_id").empty();
                $("#tagihan_supplier_id").append(`<option value="#">Choose...</option>`);

                if (!isNaN($('#supplier_id option:selected').val())) {                
            ...
            if($('#payment_mode_id option:selected').val()==2 || $('#payment_mode_id option:selected').val()==3){
                $('#other-fee').css('display','block');
            }else{
                $('#other-fee').css('display','none');
            }
        });
