...
<div class="page-wrapper">
    <div class="page-content">
        ...
        <!--end breadcrumb-->
        <div class="row">
            <form id="submit-form" action="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/'.$folder.'/'.urlencode($journals->general_journal_no)) }}" method="POST" enctype="application/x-www-form-urlencoded">
                @csrf
                @method('PUT')
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-9">
                                    <label for="" class="col-form-label part-id">{{ $journals->general_journal_no }}</label>
                                    <input type="hidden" name="general_journal_no_ori" class="form-control @error('general_journal_no_ori') is-invalid @enderror"
                                        value="@if (old('general_journal_no_ori')){{ old('general_journal_no_ori') }}@else{{ $journals->general_journal_no }}@endif">
                                    @error('general_journal_no_ori')
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-3">
                                    <input readonly type="text" class="form-control @error('journal_date') is-invalid @enderror" maxlength="10"
                                        ...
                                        value="@if (old('journal_date')){{ old('journal_date') }}@else{{ date_format(date_create($journals->general_journal_date), 'd/m/Y') }}@endif">
                                    @error('journal_date')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                    <input type="hidden" class="form-control @error('totalDebet') is-invalid @enderror"
                                        name="totalDebet" id="totalDebet" value="@if (old('totalDebet')){{ str_replace(".00","",old('totalDebet')) }}@else{{ str_replace(".00","",$journals->total_debit) }}@endif">
                                    <input type="hidden" class="form-control @error('totalCredit') is-invalid @enderror"
                                        name="totalCredit" id="totalCredit" value="@if (old('totalCredit')){{ str_replace(".00","",old('totalCredit')) }}@else{{ str_replace(".00","",$journals->total_kredit) }}@endif">
                                </div>
                            </div>
                        </div>
                    </div>
                                        ...
                                        @for ($i = 0; $i < $totRow; $i++)
                                            <tr id="row{{ $i }}">
                                                <th scope="row" style="text-align:right;">
                                                    ...
                                                    <input type="hidden" name="gj_dtl_id{{ $i }}" value="@if (old('gj_dtl_id'.$i)){{ old('gj_dtl_id'.$i) }}@else{{ 0 }}@endif">
                                                </th>
                                                <td>
                                                ...
                                                <td>
                                                    <input onchange="sumDebet();" onkeyup="formatPartPrice($(this));" type="text" style="text-align: right;" class="form-control @error('debet_amount'.$i) is-invalid @enderror"
                                                        id="debet_amount{{ $i }}" name="debet_amount{{ $i }}" maxlength="22" value="@if (old('debet_amount'.$i)){{ old('debet_amount'.$i) }}@endif" />
                                                    @error('debet_amount'.$i)
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                    @enderror
                                                </td>
                                                <td>
                                                    <input onchange="sumCredit();" onkeyup="formatPartPrice($(this));" type="text" style="text-align: right;" class="form-control @error('credit_amount'.$i) is-invalid @enderror"
                                                        id="credit_amount{{ $i }}" name="credit_amount{{ $i }}" maxlength="22" value="@if (old('credit_amount'.$i)){{ old('credit_amount'.$i) }}@endif" />
                                    ...
                                    @else
                                        ...
                                        @foreach ($journaldtls as $dtls)
                                            <tr id="row{{ $i }}">
                                                ...
                                                <td>
                                                    <textarea class="form-control" name="desc_part{{ $i }}" id="desc_part{{ $i }}" rows="3"
                                                        style="width: 100%;">{{ $dtls->description }}</textarea>
                                                    @error('desc_part'.$i)
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                    @enderror
                                                </td>
                                                <td>
                                                    <input onchange="sumDebet();" onkeyup="formatPartPrice($(this));" type="text" style="text-align: right;"
                                                        ...
                                                        id="debet_amount{{ $i }}" name="debet_amount{{ $i }}" maxlength="22" value="{{ number_format($dtls->debit,0,".",",") }}" />
                                                    @error('debet_amount'.$i)
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                    @enderror
                                                </td>
                                                <td>
                                                    <input onchange="sumCredit();" onkeyup="formatPartPrice($(this));" type="text" style="text-align: right;"
                                                        ...
                                                        id="credit_amount{{ $i }}" name="credit_amount{{ $i }}" maxlength="22" value="{{ number_format($dtls->kredit,0,".",",") }}" />
                                                    @error('credit_amount'.$i)
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                    @enderror
                                                </td>
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <td style="text-align: right;">
                                            <label for="" id="lbl-total-debet">@if (old('totalDebet')){{ $qCurrency->string_val.number_format(old('totalDebet'),0,'.',',') }}@else{{ $qCurrency->string_val.number_format($totalDebet,0,'.',',') }}@endif</label>
                                        </td>
                                        <td style="text-align: right;">
                                            <label for="" id="lbl-total-credit">@if (old('totalCredit')){{ $qCurrency->string_val.number_format(old('totalCredit'),0,'.',',') }}@else{{ $qCurrency->string_val.number_format($totalKredit,0,'.',',') }}@endif</label>
                                        </td>
                                        <td>&nbsp;</td>
                                    </tr>
                                    @error('totalCredit')
                        ...
                        <div class="card-body">
                            <div class="row mb-3">
                                <div class="col-sm-12">
                                    ...
                                    @if ($journals->created_by==Auth::user()->id && $journals->active=='Y' && strpos($journals->general_journal_no,"Draft")>0)
...
<script>
    ...
    function addPart(){
        ...
        let vHtml = '<tr id="row'+totalRow+'">'+
            ...
            '<input type="hidden" name="gj_dtl_id'+totalRow+'" value="0">'+
            '</th>'+
            '<td>'+
            ...
            '<td>'+
            '<input onchange="sumDebet();" onkeyup="formatPartPrice($(this));" type="text" style="text-align: right;" class="form-control" id="debet_amount'+totalRow+'" name="debet_amount'+totalRow+'" maxlength="22" value="0" />'+
            '</td>'+
            '<td>'+
            '<input onchange="sumCredit();" onkeyup="formatPartPrice($(this));" type="text" style="text-align: right;" class="form-control" id="credit_amount'+totalRow+'" name="credit_amount'+totalRow+'" maxlength="22" value="0" />'+
            '</td>'+
    ...
    $(document).ready(function() {
        ...
        @if ($journals->created_by==Auth::user()->id && $journals->active=='Y' && strpos($journals->general_journal_no,"Draft")>0)
            $("#del-btn").click(function() {
                let msg = 'The following General Journal Numbers will be canceled.\n{{ $journals->general_journal_no }}\nContinue?';
                if(!confirm(msg)){
                    event.preventDefault();
                }else{
                    $("#generalJournalId").val('{{ $journals->id }}');
                    $("input[name='_method']").val('POST');
                    $('#submit-form').attr('method', "POST");
                    $('#submit-form').attr('action', "{{ url('/del_general_journal') }}");
                    $("#submit-form").submit();
                }
            });
        @endif
