                                ...
                                <div class="border p-4 rounded">
                                    <div class="col-xl-6">
                                            ...
                                            <div class="row mb-3">
                                                ...
                                                <div class="col-sm-9">
                                                    <select class="form-select single-select @error('branch_id') is-invalid @enderror" id="branch_id" name="branch_id">
                                                        ...
                                                        @php
                                                            $branch_id = (old('branch_id')?old('branch_id'):$stockDisAsm->branch_id);
                                                        @endphp
                                        ...
                                        <div class="row mb-3">
                                            ...
                                            <div class="col-sm-9">
                                                <select class="form-select partsAjax @error('part_id') is-invalid @enderror" id="part_id" name="part_id">
                                                    <option value="#">Choose...</option>
                                                    @php
                                                        $part_id = (old('part_id')?old('part_id'):$stockDisAsm->part_id);
                                                    ...
                                                    @foreach ($partList as $p)
                                                        ...
                                                        @endphp
                                                        <option @if ($part_id==$p->id) {{ 'selected' }} @endif value="{{ $p->id }}">{{ $partNumber }}</option>
                                                    @endforeach
                                                </select>
                                                @error('part_id')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                        ...
                                        <div class="row mb-3">
                                            ...
                                            <label for="" id="part_name" class="col-sm-9 col-form-label">@if (old('part_id')){{ $oldParts->part_name }}@else{{ $stockDisAsm->part->part_name }}@endif</label>
                                        </div>
                                        <div class="row mb-3">
                                            <label for="" class="col-sm-3 col-form-label">AVG Cost (Rp)</label>
                                            <label for="" id="avg-cost-part-to-be-disassembly" class="col-sm-9 col-form-label">@if (old('part_id')){{ number_format($oldParts->avg_cost*old('disasm_part_qty'),0,'.',',') }}@else{{ number_format($stockDisAsm->avg_cost*$stockDisAsm->qty,0,'.',',') }}@endif</label>
                                            <input type="hidden" name="avg_cost_part_to_be_disassembly_val" id="avg_cost_part_to_be_disassembly_val"
                                                value="@if (old('avg_cost_part_to_be_disassembly_val')){{ old('avg_cost_part_to_be_disassembly_val') }}@else{{ number_format($stockDisAsm->avg_cost*$stockDisAsm->qty,0,',','') }}@endif">
                                            <input type="hidden" name="avg_cost_part_to_be_disassembly_val_tmp" id="avg_cost_part_to_be_disassembly_val_tmp"
                                                value="@if (old('avg_cost_part_to_be_disassembly_val_tmp')){{ old('avg_cost_part_to_be_disassembly_val_tmp') }}@else{{ $stockDisAsm->avg_cost }}@endif">
                                        </div>
                                        <div class="row mb-3">
                                            ...
                                            <div class="col-sm-3">
                                                <input type="text" name="disasm_part_qty" id="disasm_part_qty" class="form-control @error('disasm_part_qty') is-invalid @enderror"
                                                    value="@if (old('disasm_part_qty')){{ old('disasm_part_qty') }}@else{{ $stockDisAsm->qty }}@endif" style="text-align: right;">
                                        ...
                                        <div class="row mb-3">
                                            ...
                                            <div class="col-sm-9">
                                                <textarea name="remark_txt" id="remark_txt" rows="3" class="form-control @error('remark_txt') is-invalid @enderror"
                                                    style="width:100%;">@if (old('remark_txt')){{ old('remark_txt') }}@else{{ $stockDisAsm->remark }}@endif</textarea>
                                                @error('remark_txt')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                        ...
                                        @for ($i = 0; $i < $totRow; $i++)
                                            @if (old('part_no_'.$i))
                                                <tr id="row{{ $i }}">
                                                    ...
                                                    @if($partNo)
                                                        @php
                                                            $new_cost = str_replace(",","",old('new_cost'.$i));
                                                            $total_cost += $new_cost*old('qty'.$i);
                                                            $avg_cost = (($partNo->avg_cost*$partNo->qty)+($new_cost*old('qty'.$i)))/($partNo->qty+old('qty'.$i));
                                                        @endphp
                                                        <th scope="row" style="text-align:right;">
                                                            <label for="" id="row-no{{ $i }}" class="col-form-label">{{ $i+1 }}.</label>
                                                            <input type="hidden" name="sd_part_id{{ $i }}" id="sd_part_id{{ $i }}"
                                                                value="@if (old('sd_part_id'.$i)){{ old('sd_part_id'.$i) }}@endif">
                                                        </th>
                                                        <td>
                                                        ...
                                                        <td>
                                                            <input onkeyup="multiplicationCost({{ $i }});formatAmount($(this));" type="text" class="form-control @error('new_cost'.$i) is-invalid @enderror" id="new_cost{{ $i }}" name="new_cost{{ $i }}"
                                                                maxlength="20" value="@if (old('new_cost'.$i)){{ old('new_cost'.$i) }}@endif" style="text-align: right;" />
                                                            @error('new_cost'.$i)
                                                                <div class="invalid-feedback">{{ $message }}</div>
                                                            @enderror
                                                        </td>
                                                        <td style="text-align: right;">
                                                            <label for="" id="total-cost-{{ $i }}" class="col-form-label">@if (old('part_no_'.$i)){{ number_format((old('qty'.$i)*$new_cost),0,'.',',') }}@endif</label>
                                                        </td>
                                                        <td style="text-align: right;">
                                        ...
                                        @foreach ($stockDisAsmPart as $sd)
                                            <tr id="row{{ $i }}">
                                                ...
                                                @if($partNo)
                                                    ...
                                                    {{-- <td>
                                                        <label for="" id="part-name-{{ $i }}" class="col-form-label">{{ $partNo->part_name }}</label>
                                                    </td> --}}
                                                    <td>
                                                        <input onchange="multiplicationCost({{ $i }});" type="text" class="form-control @error('qty'.$i) is-invalid @enderror"
                                                            id="qty{{ $i }}" name="qty{{ $i }}"  maxlength="5" value="{{ $sd->qty }}" style="text-align: right;" />
                                                        @error('qty'.$i)
                                                            <div class="invalid-feedback">{{ $message }}</div>
                                                        @enderror
                                                    </td>
                                                    ...
                                                    <td>
                                                        <label for="" id="part-unit-{{ $i }}" class="col-form-label">{{ $partNo->quantity_type->string_val }}</label>
                                                    </td>
                                                    <td>
                                                        <input onkeyup="multiplicationCost({{ $i }});formatAmount($(this));" type="text" class="form-control @error('new_cost'.$i) is-invalid @enderror"
                                                            id="new_cost{{ $i }}" name="new_cost{{ $i }}" maxlength="20" value="{{ number_format($sd->cost,0,'.',',') }}"
                                                    ...
                                                    <td style="text-align: right;">
                                                        <label for="" id="total-cost-{{ $i }}" class="col-form-label">{{ number_format($sd->cost*$sd->qty,0,'.',',') }}</label>
                                                    </td>
                                                    <td style="text-align: right;">
                                                        <label for="" id="avg-cost-{{ $i }}" class="col-form-label">{{ number_format($avg_cost,0,'.',',') }}</label>
                                                    </td>
                                                    <td style="text-align: center;">
                                                        <input type="checkbox" id="rowCheck{{ $i }}" value="{{ $i }}">
                                                    </td>
                        ...
                        <div class="card-body">
                            <div class="row mb-3">
                                <div class="col-sm-12">
                                    ...
                                    @if ($stockDisAsm->created_by==Auth::user()->id && $stockDisAsm->active=='Y' && strpos($stockDisAsm->stock_disassembly_no,'Draft')>0)
...
<script>
    ...
    function multiplicationCost(idx){
        ...
        if(!isNaN($("#qty"+idx).val()) && newCost && $("#part-no-"+idx).val()!=='#'){
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if(typeof o[0]!=='undefined'){
                        ...
                        if(qtyIdx===''){qtyIdx = 0;}
                        let avg_cost = ((o[0].avg_cost*o[0].total_qty)+(parseInt(qtyIdx)*parseFloat(newCost)))/(parseInt(o[0].total_qty)+parseInt(qtyIdx));
    ...
    $(document).ready(function() {
        ...
        @if ($stockDisAsm->created_by==Auth::user()->id && $stockDisAsm->active=='Y' && strpos($stockDisAsm->stock_disassembly_no,'Draft')>0)
            $("#del-btn").click(function() {
                let msg = 'The following Stock DisAssembly Numbers will be canceled.\n{{ $stockDisAsm->stock_disassembly_no }}\nContinue?';
                if(!confirm(msg)){
                    event.preventDefault();
                }else{
                    $("#orderId").val('{{ $stockDisAsm->id }}');
                    $("input[name='_method']").val('POST');
                    $('#submit-form').attr('method', "POST");
                    $('#submit-form').attr('action', "{{ url('/del_stockdisassembly') }}");
                    $("#submit-form").submit();
                }
            });
        @endif
        ...
        $("#btn-add-row").click(function() {
            ...
            let vHtml =
                ...
                '<th scope="row" style="text-align:right;">'+
                    ...
                    '<input type="hidden" name="sd_part_id'+totalRow+'" id="sd_part_id'+totalRow+'">'+
                '</th>' +
                '<td>'+
                ...
                '<td>'+
                    '<input onkeyup="multiplicationCost('+totalRow+');formatAmount($(this));" type="text" class="form-control" '+
                    'id="new_cost'+totalRow+'" name="new_cost'+totalRow+'" maxlength="22" style="text-align: right;" value="" />'+
                '</td>'+
                '<td style="text-align: right;"><label for="" id="total-cost-'+totalRow+'" class="col-form-label"></label></td>' +
