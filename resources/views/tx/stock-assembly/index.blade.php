                        ...
                        <table class="table table-striped table-bordered" id="tx-stock-assembly" style="width:100%">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        {{ $o->stock_assembly_no }}
                                        <input type="hidden" name="stock_no{{ $i }}" id="stock_no{{ $i }}" value="{{ $o->stock_assembly_no }}">
                                    </td>
                                    <td>{{ date_format(date_create($o->stock_assembly_date), 'd/m/Y') }}</td>
                                    <td>{{ $o->createdBy->userDetail->initial }}</td>
                                    <td>
                                        @if (($o->created_by==Auth::user()->id || $is_director_now=='Y' || $is_branch_head_now=='Y'))
                                            @if ($o->active=='Y' && strpos($o->stock_assembly_no,'Draft')>0)
                                            ...
                                            @if ($o->active=='Y' && strpos($o->stock_assembly_no,'Draft')==0)
                                                | <a href="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/stock-assembly-print/'.$o->id) }}" target="_new" style="text-decoration: underline;">Print</a> |
                                                <a download="" href="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/stock-assembly-print/'.$o->id) }}" target="_new" style="text-decoration: underline;">Download</a>
                                            @endif
                                        @else
                                            <a href="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/'.$folder.'/'.$o->id) }}" style="text-decoration: underline;">View</a>
                                        @endif
                                    ...
                                    {{-- <td>
                                        @if ($o->created_by==Auth::user()->id && $o->active=='Y' && strpos($o->stock_assembly_no,'Draft')>0)
