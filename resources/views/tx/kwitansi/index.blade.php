...
<div class="page-wrapper">
    <div class="page-content">
        ...
        <hr />
        <form name="form_del" id="form-del" action="{{ url('/del_kwitansi') }}" method="POST" enctype="application/x-www-form-urlencoded">
            @csrf
                        ...
                        <table class="table table-striped table-bordered" id="inventory-list" style="width:100%">
                            ...
                            <tbody>
                                @php
                                    $i = 0;
                                @endphp
                                @foreach ($kwitansis as $o)
                                    <tr>
                                        <td>
                                            {{ $o->kwitansi_no }}
                                            <input type="hidden" name="kwitansi_no{{ $i }}" id="kwitansi_no{{ $i }}" value="{{ $o->kwitansi_no }}">
                                            <input type="hidden" name="kwitansi_id_{{ $i }}" id="kwitansi_id_{{ $i }}" value="{{ $o->id }}">
                                        </td>
                                        <td>{{ date_format(date_create($o->kwitansi_date), 'd/m/Y') }}</td>
                                        <td>{{ !is_null($o->customer)?$o->customer->name:'' }}</td>
                                        <td style="text-align: right;">{{ number_format($o->np_total,0,'.',',') }}</td>
                                        ...
                                        <td>
                                            @if (($o->created_by==Auth::user()->id || $is_director_now=='Y' || $is_branch_head_now=='Y') && $o->active=='Y')
                                                ...
                                                @endif
                                                @if (is_null($o->approved_by) || strpos($o->kwitansi_no,'Draft')>0)
                                                    <a href="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/'.$folder.'/'.$o->id.'/edit') }}" style="text-decoration: underline;">Edit</a> |
                                                    <a href="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/'.$folder.'/'.$o->id) }}" style="text-decoration: underline;">View</a>
                                                @endif
                                            @else
                                        ...
                                        <td>
                                            @if($o->active=='Y' && strpos($o->kwitansi_no,'Draft')==0 && is_null($o->approved_by) && is_null($o->canceled_by))
                                                {{ 'Created' }}
                                            @endif
                                            @if($o->active=='Y' && strpos($o->kwitansi_no,'Draft')>0)
                                                {{ 'Draft' }}
                                            @endif
...
<script>
    function printDoc(i,print_type){
        ...
        $('#print-id').val(i);
        let downloadLInk = '<a '+dl+' href="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/kwitansi-print?inv=') }}'+$('#print-id').val()+'&doc=1&p='+print_type+'" target="_new" class="btn btn-light">Permohonan Pembayaran</a>&nbsp;'+
            '<a '+dl+' href="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/kwitansi-print?inv=') }}'+$('#print-id').val()+'&doc=2&p='+print_type+'" target="_new" class="btn btn-light">Tanda Terima</a>&nbsp;'+
            '<a '+dl+' href="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/kwitansi-print?inv=') }}'+$('#print-id').val()+'&doc=3&p='+print_type+'" target="_new" class="btn btn-light">Kwitansi</a>';
