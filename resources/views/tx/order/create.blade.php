                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-9">
                                    <select class="form-select single-select @error('quotation_id') is-invalid @enderror"
                                        id="quotation_id" name="quotation_id">
                                        <option value="#">Choose...</option>
                                        @php
                                            $quotationId = (old('quotation_id')?old('quotation_id'):0);
                                        @endphp
                                        @foreach ($quotations as $p)
                                        <option @if($quotationId==$p->pq_id) {{ 'selected' }} @endif
                                            value="{{ $p->pq_id }}">{{ $p->quotation_no }}</option>
                                        @endforeach
                                    </select>
                                    @error('quotation_id')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-9">
                                    <select class="form-select single-select @error('currency_id') is-invalid @enderror" id="currency_id" name="currency_id">
                                        <option value="#">Choose...</option>
                                        @php
                                            $currencyId = (old('currency_id')?old('currency_id'):0);
                                        @endphp
                                        @foreach ($currency as $b)
                                            <option @if($currencyId==$b->currency_id){{ 'selected' }}@endif value="{{ $b->currency_id }}">{{ $b->currency->title_ind }}</option>
                                        @endforeach
                                    </select>
                                    @error('currency_id')
                                        <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-9">
                                    <input readonly type="text" class="form-control @error('est_supply_date') is-invalid @enderror"
                                        maxlength="10" id="est_supply_date" name="est_supply_date" placeholder="Estimated Supply Date"
                                        value="@if(old('est_supply_date')){{ old('est_supply_date') }}@endif">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-9">
                                    ...
                                    @if(old('vat'))
                                        ...
                                        @endif
                                    @endif
                                    <input class="form-check-input" type="checkbox" id="vat" name="vat" onclick="calcGrandTotal();"
                                ...
                                <thead>
                                    <tr style="width: 100%;">
                                        ...
                                        @if(old('supplier_type_id'))
                                            ...
                                            @endif
                                        @endif
                                        <th id="price-rp" style="width: 20%;">{{ $price_rp }}</th>
                                        <th id="price-rp-total" style="width: 10%;">{{ $price_rp_total }}</th>
                                        <th style="width: 15%;">Description</th>
                                        <th style="width: 10%;">Final&nbsp;Cost/FOB</th>
                                        <th style="width: 5%;">OH</th>
                                        <th style="width: 5%;">OO</th>
                                        <th style="width: 3%;text-align:center;">Del</th>
                                    </tr>
                                </thead>
                                <tbody id="new-row">
                                    @php
                                        $iRowPart = 1;
                                        $vatVal = old('vat') == 'on' ? $qVat->numeric_val : 0;
                                        $totalAmount = 0;
                                    @endphp
                                    @for ($i = 0; $i < $totRow; $i++)
                                        @if(old('part_id'.$i))
                                            <tr id="row{{ $i }}">
                                                <th scope="row" style="text-align:right;"><label for="" id="purchase_order_row_number{{ $i }}" class="col-form-label">{{ $iRowPart }}.</label></th>
                                                @php
                                                ...
                                                @endphp
                                                <td>
                                                    <select class="form-select partsAjax @error('part_id'.$i) is-invalid @enderror" id="part_id{{ $i }}" name="part_id{{ $i }}"
                                                        onchange="dispPriceRef(this.value, {{ $i }});">
                                                        <option value="#">Choose...</option>
                                                        @php
                                                ...
                                                <td>
                                                    <input onchange="totalPrice({{ $i }});" type="text" style="text-align: right;"
                                                        ...
                                                        id="qty{{ $i }}" name="qty{{ $i }}" maxlength="6" value="@if(old('qty'.$i)){{ old('qty'.$i) }}@endif" />
                                                    @error('qty'.$i)
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                    @enderror
                                                </td>
                                                ...
                                                <td>
                                                    <textarea name="desc_part{{ $i }}" id="desc_part{{ $i }}" rows="3" class="form-control @error('desc_part'.$i) is-invalid @enderror"
                                                        id="desc_part{{ $i }}" style="width: 100%;">@if(old('desc_part'.$i)){{ old('desc_part'.$i) }}@endif</textarea>
                                                    @error('desc_part'.$i)
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                    @enderror
                                                </td>
                                                ...
                                                <td style="text-align: right;">
                                                    <label id="oh-{{ $i }}" for="" class="col-form-label">{{ $q?number_format($q->total_qty,0,'.',','):0 }}</label>
...
<script>
    ...
    function dispPriceRef(part_id, idx){
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                $('#final-cost-'+idx).html(parseFloat(o[0].final_cost).numberFormat(0,'.',',')+' / '+fob_curr+parseFloat(final_fob).numberFormat(0,'.',','));
    ...
    function addPart(){
        ...
        let vHtml = '<tr id="row'+totalRow+'">'+
            ...
            '</td>'+
            '<td>'+
            '<input onchange="totalPrice('+totalRow+');" type="text" class="form-control" style="text-align: right;" id="qty'+totalRow+'" name="qty'+totalRow+'" maxlength="6" />'+
            '</td>'+
            '<td><label id="unit'+totalRow+'" for="" class="col-form-label"></label></td>'+
            '<td>'+
            '<input onchange="formatPartPrice('+totalRow+');" type="text" class="form-control" style="text-align: right;" '+
            'id="price_part'+totalRow+'" name="price_part'+totalRow+'" maxlength="64" />'+
            '</td>'+
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if(o.supplier_type_id==10){
                        $('#price-rp').text('Price');
                        $('#price-rp-total').text('Total');
                    }else{
                        $('#price-rp').text('Price ({{ $qCurrency->string_val }})');
                        $('#price-rp-total').text('Total ({{ $qCurrency->string_val }})');
                    }
                },
            });
