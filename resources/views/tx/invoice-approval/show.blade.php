...
<div class="page-wrapper">
    <div class="page-content">
        ...
        <!--end breadcrumb-->
        <div class="row">
            <form id="submit-form" action="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/'.$folder.'/'.urlencode($qInv->invoice_no)) }}" method="POST" enctype="application/x-www-form-urlencoded">
                @csrf
                @method('PUT')
                                    ...
                                    <div class="row">
                                        <div class="col-xl-12">
                                            ...
                                            <div class="row mb-3">
                                                ...
                                                <label for="" class="col-sm-9 col-form-label part-id">{{ (!is_null($qInv->delivery_order)?$qInv->delivery_order->delivery_order_no:'') }}</label>
                                            </div>
                                            <div class="row mb-3">
                                            ...
                                            <div class="row mb-3">
                                                ...
                                                <label for="" class="col-sm-9 col-form-label">
                                                    @if(!is_null($qInv->approved_by) && $qInv->active=='Y')
                                                        {{ 'Approved at '.date_format(date_create($qInv->approved_at), 'd-M-Y H:i:s').' by '.$qInv->approvedBy->name }}
                                                    @endif
                                                    @if(!is_null($qInv->canceled_by) && $qInv->active=='Y')
                                                        {{ 'Rejected at '.date_format(date_create($qInv->canceled_at), 'd-M-Y H:i:s').' by '.$qInv->canceledBy->name }}
                                                    @endif
                                                    @if(is_null($qInv->approved_by) && is_null($qInv->canceled_at) && $qInv->active=='Y' && strpos($qInv->invoice_no,'Draft')==0)
                                                    ...
                                                    <div class="card">
                                                        <div class="card-body">
                                                            <table class="table table-bordered mb-0">
                                                                ...
                                                                <tbody id="new-row-so">
                                                                    @php
                                                                        $delivery_order_per_id = explode(",",(!is_null($delivery_order_per_id)?$delivery_order_per_id->sales_order_no_all:''));
                                                                        $i = 1;
                                                                    @endphp
