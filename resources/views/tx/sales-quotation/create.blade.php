                        ...
                        <div class="card-body">
                            ...
                            <div class="border p-4 rounded">
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        <select
                                            class="form-select single-select @error('customer_id') is-invalid @enderror"
                                ...
                                <div id="customer_data" class="row mb-3">
                                    ...
                                    <div id="customer_info" class="col-sm-9">
                                        @if(old('customer_id'))
                                            ...
                                            '<br />'.(!is_null($customerPics[0]->country)?$customerPics[0]->country->country_name:'').
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-select single-select @error('customer_pic') is-invalid @enderror"
                                            ...
                                            <option value="#">Choose...</option>
                                            @php
                                                $customerPic = (old('customer_pic')?old('customer_pic'):0);
                                            @endphp
                                            @foreach ($customerPics as $p)
                                                <option @if ($customerPic==1) {{ 'selected' }} @endif value="1">{{ $p->pic1_name }}</option>
                                                @if (!is_null($p->pic2_name))
                                                    <option @if ($customerPic==2) {{ 'selected' }} @endif value="2">{{ $p->pic2_name }}</option>
                                                @endif
                                            @endforeach
                                        </select>
                                        @error('customer_pic')
                                ...
                                @else
                                    <input type="hidden" id="branch_id" name="branch_id" value="@if(old('branch_id')){{ old('branch_id') }}@else{{ $userLogin->branch_id }}@endif">
                                @endif
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        <textarea name="salesHeader" id="salesHeader" rows="3" maxlength="1000" style="width: 100%;"
                                            class="form-control @error('salesHeader') is-invalid @enderror">@if (old('salesHeader')){{ old('salesHeader') }}@endif</textarea>
                                        @error('salesHeader')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        <textarea name="salesFooter" id="salesFooter" rows="3" maxlength="1000" style="width: 100%;"
                                            class="form-control @error('salesFooter') is-invalid @enderror">@if (old('salesFooter')){{ old('salesFooter') }}@endif</textarea>
                                        @error('salesFooter')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                        ...
                                        @if (old('part_id'.$i))
                                            <tr id="row{{ $i }}">
                                                ...
                                                <td style="text-align: right;">
                                                    <label id="part_type-{{ $i }}" for="" class="col-form-label">{{ (is_null($partNo)?'':$partNo->part_type->title_ind) }}</label>
                                                </td>
                                                <td>
                                                    <input type="text" class="form-control @error('qty'.$i) is-invalid @enderror"
                                                        id="qty{{ $i }}" name="qty{{ $i }}" maxlength="7"
                                                        value="@if (old('qty'.$i)){{ old('qty'.$i) }}@endif" />
                                                    @error('qty'.$i)
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                    @enderror
                                                </td>
                                                ...
                                                <td>
                                                    <textarea class="form-control @error('desc_part'.$i) is-invalid @enderror" name="desc_part{{ $i }}" id="desc_part{{ $i }}"
                                                        rows="3" style="width: 100%;">@if (old('desc_part'.$i)){{ old('desc_part'.$i) }}@endif</textarea>
                                                    @error('desc_part'.$i)
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                    @enderror
                                                </td>
                                                <td style="text-align: right;">
                                                    <label id="final-cost-{{ $i }}" for=""
                                                        class="col-form-label">{{ (is_null($partNo)?'':number_format($partNo->avg_cost,0,'.',',')) }}</label>
                                                </td>
                                                <td style="text-align: right;">
                                                    <label id="final-price-{{ $i }}" for=""
                                                        class="col-form-label">{{ (is_null($partNo)?'':number_format($partNo->final_price,0,'.',',')) }}</label>
                                                </td>
                                                <td style="text-align: right;">
                                                    <label id="price_list-{{ $i }}" for=""
                                                        class="col-form-label">{{ (is_null($partNo)?'':number_format($partNo->price_list,0,'.',',')) }}</label>
                                                </td>
    ...
    function dispPartRef(part_id, idx){
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                $('#price_list-'+idx).text(parseFloat((o[0].price_list!=null?o[0].price_list:0)).numberFormat(0,'.',','));
            },
        });
    }
    ...
    $(document).ready(function() {
        ...
        $("#btn-add-row").click(function() {
            ...
            let vHtml = '<tr id="row'+totalRow+'">'+
                ...
                '<td><input type="text" class="form-control" onkeyup="formatAmount($(this));" id="price'+totalRow+'" name="price'+totalRow+'" '+
                'maxlength="64" style="text-align: right;" value="" /></td>'+
