                                            ...
                                            <div class="row mb-3">
                                                ...
                                                <div class="col-sm-9">
                                                    <select class="form-select single-select @error('branch_id') is-invalid @enderror"
                                                        ...
                                                        @php
                                                            $p_Id = (old('branch_id')?old('branch_id'):$queryStock->branch_id);
                                                        @endphp
                                                        @foreach ($branch as $b)
                                            ...
                                            <div class="row mb-3">
                                                ...
                                                <div class="col-sm-9">
                                                    <textarea name="remark" id="remark" rows="3" maxlength="512" class="form-control @error('remark') is-invalid @enderror"
                                                        style="width: 100%;">@if (old('remark')){{ old('remark') }}@else{{ $queryStock->remark }}@endif</textarea>
                                                    @error('remark')
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                    @enderror
                                                </div>
                                            ...
                                            @if (old('part_id'.$lastIdx))
                                                ...
                                                <tr id="row{{ $lastIdx }}">
                                                    <th scope="row" style="text-align:right;">
                                                        ...
                                                        <input type="hidden" name="adj_part_id_{{ $lastIdx }}" id="adj_part_id_{{ $lastIdx }}" value="{{ old('adj_part_id_'.$lastIdx) }}">
                                                    </th>
                                                    <td>
                                                    ...
                                                    <td style="text-align:right;">
                                                        <label id="oh_{{ $lastIdx }}" for="" class="col-form-label">{{ number_format(old('oh_ori_'.$lastIdx),0,'.',',') }}</label>
                                                        <input type="hidden" name="oh_ori_{{ $lastIdx }}" id="oh_ori_{{ $lastIdx }}" value="{{ old('oh_ori_'.$lastIdx) }}">
                                                    </td>
                                                    <td style="text-align:right;">
                                                        <label id="so_{{ $lastIdx }}" for="" class="col-form-label">{{ number_format(old('so_ori_'.$lastIdx),0,'.',',') }}</label>
                                                        <input type="hidden" name="so_ori_{{ $lastIdx }}" id="so_ori_{{ $lastIdx }}" value="{{ old('so_ori_'.$lastIdx) }}">
                                                    </td>
                                                    <td style="text-align:right;">
                                                        <label id="avg_cost_{{ $lastIdx }}" for="" class="col-form-label">{{ number_format(old('avg_cost_ori_'.$lastIdx),0,'.',',') }}</label>
                                                        <input type="hidden" name="avg_cost_ori_{{ $lastIdx }}" id="avg_cost_ori_{{ $lastIdx }}" value="{{ old('avg_cost_ori_'.$lastIdx) }}">
                                                    </td>
                                                    <td style="text-align:right;">
                                                        <label id="total_{{ $lastIdx }}" for="" class="col-form-label">
                                                            {{ number_format(($qty_adj_*old('avg_cost_ori_'.$lastIdx)),0,'.',',') }}
                                                        </label>
                                                    </td>
                                                    <td>
                                                    ...
                                                    @php
                                                        $lastTotalAmount+= ($qty_adj_*old('avg_cost_ori_'.$lastIdx));
                                                    @endphp
                                                </tr>
                                            @endif
                                        @endfor
                                    @else
                                        ...
                                        @foreach ($queryStockPart as $qP)
                                            ...
                                            <tr id="row{{ $lastIdx }}">
                                                ...
                                                <td><label id="part_type_{{ $lastIdx }}" for="" class="col-form-label">{{ $query->part_type_name }}</label></td>
                                                <td>
                                                    <input type="text" class="form-control" style="text-align: right;" id="qty_adj_{{ $lastIdx }}" name="qty_adj_{{ $lastIdx }}"
                                                        maxlength="15" value="{{ $qP->adjustment }}"
                                                        onchange="calcTotal(this.value,{{ $lastIdx }});" />
                                                    @error('qty_adj_'.$lastIdx)
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                    @enderror
                                                ...
                                                <td>
                                                    <textarea class="form-control" name="notes_{{ $lastIdx }}" id="notes_{{ $lastIdx }}" rows="3"
                                                        style="width: 100%;">{{ $qP->notes }}</textarea>
                                                </td>
                                                <td style="text-align:center;"><input type="checkbox" id="rowCheck{{ $lastIdx }}" value="{{ $lastIdx }}"></td>
                                                @php
                        ...
                        <div class="card-body">
                            <div class="row mb-3">
                                <div class="col-sm-12">
                                    ...
                                    <input type="hidden" name="stockAdjId" id="stockAdjId">
...
<script>
    ...
    function addPart(){
        ...
        let vHtml = '<tr id="row'+totalRow+'">'+
            ...
            '<input type="hidden" name="adj_part_id_'+totalRow+'" id="adj_part_id_'+totalRow+'" value="0">'+
            '</th>'+
            '<td>'+
    ...
    $(document).ready(function() {
        ...
        $("#del-btn").click(function() {
            let msg = 'The following Stock Adjustment Numbers will be canceled.\n{{ $queryStock->stock_adj_no }}\nContinue?';
            if(!confirm(msg)){
                event.preventDefault();
            }else{
                $("#stockAdjId").val('{{ $queryStock->id }}');
                $("input[name='_method']").val('POST');
                $('#submit-form').attr('method', "POST");
                $('#submit-form').attr('action', "{{ url('/del_stockadj') }}");
                $("#submit-form").submit();
            }
        });
