...
<div class="page-wrapper">
    <div class="page-content">
        ...
        <!--end breadcrumb-->
        <div class="row">
            <form id="submit-form" method="POST" enctype="application/x-www-form-urlencoded"
                action="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/'.$folder.'/'.(!is_null($qPaymentInv->payment_receipt_no)?urlencode($qPaymentInv->payment_receipt_no):urlencode($qPaymentInv->payment_receipt_plan_no))) }}">
                @csrf
                @method('PUT')
                ...
                <div class="col-xl-12 mx-auto">
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-select single-select @error('customer_id') is-invalid @enderror" id="customer_id" name="customer_id">
                                            ...
                                            @php
                                                $customerId = (old('customer_id')?old('customer_id'):$qPaymentInv->customer_id);
                                            @endphp
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        <input onkeyup="formatPartPrice($(this));" type="text" class="form-control @error('total_payment') is-invalid @enderror"
                                            ...
                                            value="@if (old('total_payment')){{ old('total_payment') }}@else{{ number_format($qPaymentInv->payment_total,0,".",",") }}@endif">
                                        @error('total_payment')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                            ...
                                            @if (old('invoice_no_'.$i))
                                                <tr id="row{{ $i }}">
                                                    <th scope="row" style="text-align:right;">
                                                        ...
                                                        <input type="hidden" name="payment_receipt_inv_id{{ $i }}" id="payment_receipt_inv_id{{ $i }}"
                                                            value="@if (old('payment_receipt_inv_id'.$i)){{ old('payment_receipt_inv_id'.$i) }}@endif">
                                                    </th>
                                                    <td>
                                                        @php
                                                            $inv_no = '';
                                                    ...
                                                    <td style="text-align: right;">{{ number_format(old('total_inv_o_'.$i)-(str_replace(",","",old('total_inv_'.$i))),0,".",",") }}</td>
                                    ...
                                    @else
                                        ...
                                        @foreach ($queryInv as $qInv)
                                            <tr id="row{{ $i }}">
                                                <th scope="row" style="text-align:right;">
                                                    <label for="" id="payment_receipt_row_number{{ $i }}" class="col-form-label">{{ $i+1 }}.</label>
                                                    <input type="hidden" name="payment_receipt_inv_id{{ $i }}" id="payment_receipt_inv_id{{ $i }}" value="{{ $qInv->id }}">
                                                </th>
                                                <td>
                                                    @php
                                                        $inv_no = '';
                                                ...
                                                <td>
                                                    <textarea class="form-control @error('desc_'.$i) is-invalid @enderror"
                                                        ...
                                                        style="width: 100%;">{!! $qInv->description !!}</textarea>
                                                    @error('desc_'.$i)
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                    @enderror
                                                </td>
                                                ...
                                                <td style="text-align: right;">
                                                    <input onkeyup="formatPartPrice($(this));" type="text" class="form-control @error('total_inv_'.$i) is-invalid @enderror"
                                                        id="total_inv_{{ $i }}" name="total_inv_{{ $i }}" maxlength="25"
                                                        value="{{ number_format($qInv->total_payment,0,".",",") }}" style="text-align: right;" />
                                                    <input type="hidden" id="total_inv_o_{{ $i }}" name="total_inv_o_{{ $i }}"
                                                        maxlength="25" value="{{ $qInv->total_payment_full }}" />
                                                    @error('total_inv_'.$i)
                                                        <div class="invalid-feedback" style="text-align: left;">{{ $message }}</div>
                                                    @enderror
                                                </td>
                                                <td style="text-align: right;">{{ number_format($qInv->total_payment_full-$qInv->total_payment,0,".",",") }}</td>
                                                <td style="text-align: center;"><input type="checkbox" id="rowCheck{{ $i }}" value="{{ $i }}"></td>
                                            </tr>
                                            @php
                                    ...
                                    <tr>
                                        ...
                                        <td style="text-align: right">
                                            <label for="" id="tot-biaya-lainnya">
                                                ...
                                                @else
                                                    @php
                                                        $diskon_pembelian = $qPaymentInv->diskon_pembelian;
                                                        $admin_bank = $qPaymentInv->admin_bank;
                                                        $biaya_kirim = $qPaymentInv->biaya_kirim;
                                                        $penerimaan_lainnya = $qPaymentInv->penerimaan_lainnya;
                                                    @endphp
                                                    {{ number_format($biaya_kirim+$penerimaan_lainnya-$diskon_pembelian-$admin_bank,0,".",",") }}
                                                @endif
                                            </label>
                    ...
                    <div id="other-fee" class="card" style="margin-top: 15px;">
                        @php
                            ...
                            $bgColor = "#fff";
                        @endphp
                        @if (old('payment_mode_id')==1 || $qPaymentInv->payment_mode==1)
                            @php
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-6">
                                    <input {{ $readonlyOtherFee }} onkeyup="formatPartPrice($(this));" type="text" class="form-control @error('admin_bank') is-invalid @enderror"
                                        id="admin_bank" name="admin_bank" placeholder="0"
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-6">
                                    <input onkeyup="formatPartPrice($(this));" type="text" class="form-control @error('penerimaan_lainnya') is-invalid @enderror"
                                        id="penerimaan_lainnya" name="penerimaan_lainnya" placeholder="0" 
                                        value="@if(old('penerimaan_lainnya')){{ old('penerimaan_lainnya') }}@else{{ number_format($qPaymentInv->penerimaan_lainnya,0,".",",") }}@endif">
                        ...
                        <div class="card-body">
                            <div class="row mb-3">
                                <div class="col-sm-12">
                                    ...
                                    @if ($qPaymentInv->created_by==Auth::user()->id && $qPaymentInv->active=='Y' && is_null($qPaymentInv->approved_by))
...
<script>
    ...
    function sumTerbayar(){
        ...
        let penerimaan_lainnya = $('#biaya_kirim').val().replaceAll(',','');
    ...
    $(document).ready(function() {
        ...
        @if ($qPaymentInv->created_by==Auth::user()->id && $qPaymentInv->active=='Y' && is_null($qPaymentInv->approved_by))
            $("#del-btn").click(function() {
                let msg = 'The following Payment Receipt Numbers will be canceled.\n{{ $qPaymentInv->payment_receipt_no }}\nContinue?';
                if(!confirm(msg)){
                    event.preventDefault();
                }else{
                    $("#payment_receiptId").val('{{ $qPaymentInv->id }}');
                    $("input[name='_method']").val('POST');
                    $('#submit-form').attr('method', "POST");
                    $('#submit-form').attr('action', "{{ url('/del_preceipt') }}");
                    $("#submit-form").submit();
                }
            });
        @endif
        ...
        $("#btn-add-row").click(function() {
            ...
            var fd = new FormData();
            fd.append('pc_id', {{ $qPaymentInv->id }});
