                                ...
                                <div class="border p-4 rounded">
                                    <div class="col-xl-6">
                                        ...
                                        <div class="row mb-3">
                                            ...
                                            <label for="" class="col-sm-9 col-form-label">{{ date_format(date_create($qStock->stock_transfer_date), 'd/m/Y') }}</label>
                                        </div>
                                        <div class="row mb-3">
                                            ...
                                            <div class="col-sm-9">
                                                <select class="form-select single-select @error('branch_from_id') is-invalid @enderror"
                                                    ...
                                                    @php
                                                        $branch_from_id = (old('branch_from_id')?old('branch_from_id'):$qStock->branch_from_id);
                                                    @endphp
                                        ...
                                        <div class="row mb-3">
                                            ...
                                            <div class="col-sm-9">
                                                <select class="form-select single-select @error('branch_to_id') is-invalid @enderror"
                                                    ...
                                                    <option value="#">Choose...</option>
                                                    @php
                                                        $branch_to_id = (old('branch_to_id')?old('branch_to_id'):$qStock->branch_to_id);
                                                    @endphp
                                        ...
                                        <div class="row mb-3">
                                            ...
                                            <div class="col-sm-9">
                                                <textarea name="remark_txt" id="remark_txt" rows="3" class="form-control @error('remark_txt') is-invalid @enderror"
                                                    style="width:100%;">@if (old('remark_txt')){{ old('remark_txt') }}@else{{ $qStock->remark }}@endif</textarea>
                                                @error('remark_txt')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                            ...
                            <table class="table table-bordered mb-0">
                                <thead>
                                    <tr style="width: 100%;">
                                        ...
                                        <th scope="col" style="width: 65%;">Part</th>
                                            ...
                                            @if (old('part_no_'.$i))
                                                <tr id="row{{ $i }}">
                                                    <th scope="row" style="text-align:right;">
                                                        ...
                                                        <input type="hidden" name="part_row_id{{ $i }}" id="part_row_id{{ $i }}" value="@if (old('part_row_id')){{ old('part_row_id') }}@endif">
                                                    </th>
                                                    @php
                                                    ...
                                                    @endphp
                                                    <td>
                                                        <select onchange="dispPartRef_No(this.value, {{ $i }});" class="form-select partsAjax @error('part_no_'.$i) is-invalid @enderror"
                                    ...
                                    @else
                                        ...
                                        @foreach ($qStockPart as $qSPart)
                                            <tr id="row{{ $i }}">
                                                ...
                                                <td>
                                                    <input type="text" class="form-control @error('qty'.$i) is-invalid @enderror" id="qty{{ $i }}" name="qty{{ $i }}"
                                                        maxlength="5" value="{{ $qSPart->qty }}" style="text-align: right;" />
                                                    @error('qty'.$i)
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                    @enderror
                                                </td>
                        ...
                        <div class="card-body">
                            <div class="row mb-3">
                                <div class="col-sm-12">
                                    ...
                                    @if ($qStock->created_by==Auth::user()->id && $qStock->active=='Y' && is_null($qStock->approved_by))
    ...
    $(document).ready(function() {
        ...
        @if ($qStock->created_by==Auth::user()->id && $qStock->active=='Y' && is_null($qStock->approved_by))
            $("#del-btn").click(function() {
                let msg = 'The following Stock Transfer Numbers will be canceled.\n{{ $qStock->stock_transfer_no }}\nContinue?';
                if(!confirm(msg)){
                    event.preventDefault();
                }else{
                    $("#orderId").val('{{ $qStock->id }}');
                    $("input[name='_method']").val('POST');
                    $('#submit-form').attr('method', "POST");
                    $('#submit-form').attr('action', "{{ url('/del_stocktransfer') }}");
                    $("#submit-form").submit();
                }
            });
        @endif
        ...
        $("#btn-add-row").click(function() {
            ...
            let vHtml =
                '<tr id="row' + totalRow + '">' +
                '<th scope="row" style="text-align:right;">' +
                '<label for="" id="" class="col-form-label">' + rowNo + '.</label>' +
                '<input type="hidden" name="part_row_id'+totalRow+'" id="part_row_id'+totalRow+'" value="9999999">' +
                '</th>' +
                '<td>'+
