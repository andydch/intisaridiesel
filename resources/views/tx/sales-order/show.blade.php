                                    ...
                                    @foreach ($order_parts as $op)
                                        ...
                                        <tr id="row{{ $i }}">
                                            ...
                                            @php
                                                ...
                                                // $qtySO = 0;
                                                // $so = \App\Models\Tx_sales_order_part::leftJoin('tx_sales_orders AS txso','tx_sales_order_parts.order_id','=','txso.id')
                                                ...
                                                //     $qDO = \App\Models\Tx_delivery_order::where('delivery_order_no','NOT LIKE','%Draft%')
                                                //     ->where('sales_order_no_all','LIKE','%'.$so_part->sales_order_no.'%')
                                                //     ->where('active','=','Y')
                                                //     ->first();
                                                //     if(!$qDO){
                                                //         $qtySO += $so_part->qty;
                                                ...
                                                // sales order
                                                    ...
                                                    // ->when($orders->branch_id!=null, function($q) use($orders) {
                                                    //     $q->where('usr.branch_id','=',$orders->branch_id==null);
                                                    // })
                                            ...
                                            @php
                                                $qLastFinalPrice = \App\Models\Tx_sales_order_part::selectRaw('IFNULL(tx_sales_order_parts.price,0) AS last_final_price')
                                                ...
                                                ->where('tx_sales_order_parts.active','=','Y')
                                                // ->where('txso.need_approval','=','N')
                                                // ->where('txso.id','<',$op->order_id)
                                                ->where('txso.active','=','Y')
                                                ->orderBy('txso.created_at','DESC')
                                                ->limit(1)
                                                ->first();
                                            @endphp
                                            {{-- @if(is_null($qLastFinalPrice))
                                                @php
                                                    $qLastFinalPrice = \App\Models\Tx_sales_order_part::selectRaw('IFNULL(tx_sales_order_parts.price,0) AS last_final_price')
                                                    ->leftJoin('tx_sales_orders as txso','tx_sales_order_parts.order_id','=','txso.id')
                                                    ->leftJoin('userdetails as usr','txso.created_by','=','usr.user_id')
                                                ...
                                                @endphp
                                            @endif --}}
                                            <td style="text-align: right;"><label id="final-price-{{ $i }}" for="" class="col-form-label">{{ $qCurrency->string_val.number_format(($qLastFinalPrice?$qLastFinalPrice->last_final_price:0),0,'.',',') }}</label></td>
