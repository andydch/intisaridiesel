                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @if (($o->created_by==Auth::user()->id || $is_director_now=='Y' || $is_branch_head_now=='Y') &&
                                                ...
                                                ($o->need_approval=='N' || strpos($o->sales_order_no,"Draft")>0))
                                                <a href="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/'.$folder.'/'.$o->id.'/edit') }}" style="text-decoration: underline;">Edit</a> |
                                                    ...
                                                    {{-- <a href="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/print-sales-order?so='.$o->id) }}" style="text-decoration: underline;">Print</a> |
                                                    <a download="" href="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/print-sales-order?so='.$o->id) }}" style="text-decoration: underline;">Download</a> | --}}
                                            @else
                                        ...
                                        <td>
                                            @if ($o->active=='N')
                                                Cancel
                                            @endif
                                            @if ($o->active=='Y' && strpos($o->sales_order_no,"Draft")>0)
                                                Draft
                                            @endif
                                            @if ($o->active=='Y' && !strpos($o->sales_order_no,"Draft") && $o->need_approval=='N' && is_null($o->approved_by)
...
<script>
    function printDoc(i,print_type){
        ...
        $('#print-id').val(i);
        let downloadLInk = '<a '+dl+' href="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/print-sales-order?so=') }}'+$('#print-id').val()+'&doc=1&p='+print_type+'" target="_new" class="btn btn-light">With Price</a>&nbsp;'+
            '<a '+dl+' href="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/print-sales-order?so=') }}'+$('#print-id').val()+'&doc=2&p='+print_type+'" target="_new" class="btn btn-light">No Price</a>';
