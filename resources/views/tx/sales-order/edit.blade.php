                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            {{-- <div class="row mb-3">
                                ...
                                <input type="hidden" name="sales_order_no" id="sales_order_no" value="{{ $orders->sales_order_no }}">
                            </div> --}}
                            <div class="row mb-3">
                                <label for="" class="col-sm-3 col-form-label">Sales Order No</label>
                                <div class="col-sm-9">
                                    <label for="" class="col-form-label part-id">
                                        {{ $orders->sales_order_no }}
                                    </label>
                                    <input type="hidden" name="sales_order_no" id="sales_order_no" class="@error('sales_order_no') is-invalid @enderror" 
                                        style="visibility: hidden;" value="{{ $orders->sales_order_no }}">
                            ...
                            @else
                            <input type="hidden" name="branch_id" id="branch_id" value="@if (old("branch_id")){{ old("branch_id") }}@else{{ $orders->branch_id }}@endif">
                            @endif
                            <div class="row mb-3">
                                ...
                                <tbody id="new-row">
                                                ...
                                                <tr id="row{{ $i }}">
                                                    ...
                                                    @php
                                                        ...
                                                        ->selectRaw('(SELECT avg_cost 
                                                            ...
                                                            AND updated_at<(SELECT created_at FROM tx_sales_orders WHERE id='.$orders->id.') 
                                                        ...
                                                        // sales order
                                                        $qLastSO = \App\Models\Tx_sales_order_part::leftJoin('tx_sales_orders AS txso','tx_sales_order_parts.order_id','=','txso.id')
                                                        ->select(
                                                            'tx_sales_order_parts.last_avg_cost AS last_avg_cost',
                                                        )
                                    ...
                                    @else
                                        ...
                                        @foreach ($order_parts as $op)
                                            ...
                                            <tr id="row{{ $i }}">
                                                <th scope="row" style="text-align:right;">
                                                    <label for="" id="sales_order_row_number{{ $i }}" class="col-form-label">{{ $i+1 }}.</label>
                                                </th>
                                                ...
                                                @php
                                                    ...
                                                    // ->where('txso.need_approval','=','N')
                                                    ->where('txso.active','=','Y')
                                                    ->when(!is_null($orders->branch_id), function($q) use($orders) {
                                                        $q->whereRaw('((usr.branch_id='.$orders->branch_id.' AND txso.branch_id IS null) OR txso.branch_id='.$orders->branch_id.')');
                                                    })
                                                    ->sum('tx_sales_order_parts.qty');
                                                    ...
                                                    ->where('txsj.active','=','Y')
                                                    ->when(!is_null($orders->branch_id), function($q) use($orders) {
                                                        $q->whereRaw('((usr.branch_id='.$orders->branch_id.' AND txsj.branch_id IS null) OR txsj.branch_id='.$orders->branch_id.')');
                                                    })
                                                    ->sum('tx_surat_jalan_parts.qty');
                                                ...
                                                <td>
                                                    ...
                                                    @endif
                                                    <input type="hidden" name="part_id{{ $i }}" id="part_id{{ $i }}" value="{{ $op->part_id }}" 
                                                        class="@error('part_id'.$i) is-invalid @enderror">
                                                    {{-- <select onchange="dispPartRef(this.value, {{ $i }});"
                                                        class="form-select partsAjax @error('part_id'.$i) is-invalid @enderror" id="part_id{{ $i }}" name="part_id{{ $i }}">
                                                        <option value="#">Choose...</option>
                                                        @php
                                                            $partId = $op->part_id;
                                                            $partList = \App\Models\Mst_part::where([
                                                ...
                                                <td style="text-align: right;"><label id="final-price-{{ $i }}" for="" class="col-form-label">{{ (is_null($partNo)?'':number_format($partNo->final_price,0,'.',',')) }}</label></td>
                                                <input type="hidden" name="avg_cost_{{ $i }}_db" id="avg_cost_{{ $i }}-db" value="{{ (!$qLastSO?0:$qLastSO->last_avg_cost) }}">
                                                <td style="text-align: right;"><label id="oh-{{ $i }}" for="" class="col-form-label">{{ number_format((!is_null($partNo)?$partNo->qty_oh:0),0,'.',',') }}</label></td>
                                                <td style="text-align: right;"><label id="so-{{ $i }}" for="" class="col-form-label">{{ number_format($qtySO,0,'.',',') }}</label></td>
                                                <td>
    ...
    $(document).ready(function() {
        ...
        @if ($orders->created_by==Auth::user()->id && $orders->active=='Y' && is_null($orders->delivery_order) && is_null($orders->approved_by))
            $("#del-btn").click(function() {
                let msg = 'The following Sales Order Numbers will be canceled.\n{{ $orders->sales_order_no }}\nContinue?';
                if(!confirm(msg)){
                    event.preventDefault();
                }else{
                    ...
                    $('#submit-form').attr('action', "{{ url('/del_salesorder') }}");
                    $("#submit-form").submit();
                }
            });
        @endif
