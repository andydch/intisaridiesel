                                        ...
                                        <td>
                                            @if (($o->created_by==Auth::user()->id || $is_director_now=='Y' || $is_branch_head_now=='Y') && $o->active=='Y')
                                                <a href="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/'.$uri.'/'.$o->id.'/edit') }}" style="text-decoration: underline;">Edit</a> |
                                                    ...
                                                    <a href="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/print-faktur?fk='.$o->id) }}" target="_new" style="text-decoration: underline;">Print</a> |
                                                    <a download="" href="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/print-faktur?fk='.$o->id) }}" target="_new" style="text-decoration: underline;">Download</a> |
                                                    <a href="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/'.$uriFp.'/'.$o->id.'/edit') }}" style="text-decoration: underline;">
                                                    @if (!is_null($o->tax_invoice_id))
                                                        Edit FP
                                                    @else
                                                        Add FP
                                                    @endif
                                            ...
                                            {{-- @if ($o->created_by==Auth::user()->id || $is_director_now=='Y' || $is_branch_head_now=='Y')
                                                @if($o->active=='Y')
                                                    @if (strpos($o->delivery_order_no,'Draft')==0)
                                                        <a href="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/delivery-order-adjustment/'.$o->id.'/edit') }}" style="text-decoration: underline;">Adjustment</a> |
                                                    @endif
                                                    ...
                                                    <a href="{{ url(ENV('TRANSACTION_FOLDER_NAME').'/print-order/'.$o->id) }}" style="text-decoration: underline;">Print</a> |
