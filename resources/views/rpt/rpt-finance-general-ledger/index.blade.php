...
<script>
    function setCoas(){
        ...
        var fd = new FormData();
        fd.append('lokal_input', $('#lokal_input').val());
        fd.append('branch_id', $('#branch_id option:selected').val());
        $.ajax({
            url: "{{ url('/disp_coa_for_gnledger_rpt') }}",
            type: "POST",
            ...
            success: function (res) {
                ...
                if (totCoas > 0) {
                    for (let i = 0; i < totCoas; i++) {
                        optionText = o[i].coa_code_complete+' - '+o[i].coa_name;
                        optionValue = o[i].id;
                        $("#coa_id").append(`<option value="${optionValue}">${optionText}</option>`);
                    }
                }
            },
        });
