                                    ...
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-sm-9">
                                            <select class="form-select single-select @error('customer_id') is-invalid @enderror"
                                                ...
                                                @php
                                                    $customer_id = (old('customer_id')?old('customer_id'):'')
                                                @endphp
                                                @foreach ($customers as $sp)
                                                    <option @if($customer_id==$sp->id){{ 'selected' }}@endif
                                                        value="{{ $sp->id }}">{{ $sp->customer_unique_code.' - '.($sp->entity_type?$sp->entity_type->string_val:'').' '.$sp->name }}</option>
                                                @endforeach
                                            </select>
                                            @error('customer_id')
