        ...
        <div class="page-content">
            ...
            <div class="row">
                <div class="col-xxl-6 col-xl-6 col-lg-6 col-md-6 col-sm-12 col-xs-12">
                    ...
                    <hr />
                    <form id="submit-form" action="{{ url(ENV('ADMIN_FOLDER_NAME').'/'.$folder.'/'.$salesman_target->id) }}" method="POST" enctype="application/x-www-form-urlencoded">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="row mb-3">
                                    <span for="" class="col-sm-3 col-form-label">Target Year</span>
                                    <span for="" class="col-sm-9 col-form-label">{{ $salesman_target->year }}</span>
                                </div>
                                <div class="row mb-3">
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9" style="margin: auto;">
                                        <input type="hidden" id="sales_target" name="sales_target" class="form-control @error('sales_target') is-invalid @enderror" maxlength="20"
                                            style="text-align: right;" value="@if (old('sales_target')){{ number_format(old('sales_target'),0,'.','') }}@else{{ number_format($salesman_target->sales_target,0,'.','') }}@endif">
                                        <span for="" id="sales_target_tmp">@if (old('sales_target')){{ $qCurrency->string_val.number_format(old('sales_target'),0,'.',',') }}@else{{ $qCurrency->string_val.number_format($salesman_target->sales_target,0,'.',',') }}@endif</span>
                                        @error('sales_target')
                                ...
                                <table class="table table-bordered mb-0">
                                    ...
                                    @if (old('totalRow'))
                                        @for ($i=0;$i<old('totalRow');$i++)
                                            @if (old('salesman_id'.$i))
                                                <tr id="row{{ $i }}">
                                                    <input type="hidden" name="sa_id{{ $i }}" value="{{ old('sa_id'.$i) }}">
                                    ...
                                    @else
                                        ...
                                        @foreach ($salesman_target_detail as $sa_dtl)
                                            <tr id="row{{ $i }}">
                                                <input type="hidden" name="sa_id{{ $i }}" value="{{ $sa_dtl->id }}">
                                                <th scope="row" style="text-align:right;">{{ $i+1 }}.</th>
                                                <td>
                                                    <select class="form-select single-select @error('salesman_id'.$i) is-invalid @enderror" id="salesman_id{{ $i }}" name="salesman_id{{ $i }}">
                                                        <option value="#">Choose...</option>
                                                        @php
                                                            $salesmanId = old('salesman_id'.$i) ? old('salesman_id'.$i) : $sa_dtl->salesman_id;
                                                        @endphp
                                                        @foreach ($salesmans as $sa)
                                                            <option @if ($salesmanId==$sa->user_id){{ 'selected' }}@endif value="{{ $sa->user_id }}">{{ $sa->name }}</option>
                                                        @endforeach
                                                ...
                                                <td>
                                                    <input type="text" class="form-control @error('sales_target_per_branch'.$i) is-invalid @enderror" id="sales_target_per_branch_tmp{{ $i }}"
                                                        name="sales_target_per_branch_tmp{{ $i }}" maxlength="20" onchange="totSalesTarget();" onkeyup="formatSalesTargetPerSales(this.value,{{ $i }});" style="text-align: right;"
                                                        value="{{ number_format($sa_dtl->sales_target_per_branch,0,'.',',') }}" />
                                                    <input type="hidden" name="sales_target_per_branch{{ $i }}" id="sales_target_per_branch{{ $i }}"
                                                        value="{{ number_format($sa_dtl->sales_target_per_branch,0,'.','') }}" />
                                                    @error('sales_target_per_branch'.$i)
                                                        <div class="invalid-feedback">{{ $message }}</div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $("#btn-add-row").click(function() {
                ...
                let vHtml = '<tr id="row'+totalRow+'"><input type="hidden" name="sa_id'+totalRow+'" value="0">'+
