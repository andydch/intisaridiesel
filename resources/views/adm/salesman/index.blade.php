@extends('layouts.app')

@section('style')
    <link href="{{ asset('assets/plugins/datatable/css/dataTables.bootstrap5.min.css') }}" rel="stylesheet" />
@endsection

@section('wrapper')
    <!--start page wrapper -->
    <div class="page-wrapper">
        <div class="page-content">
            <!--breadcrumb-->
            @include('adm.salesman.breadcrumb')
            <!--end breadcrumb-->
            <h6 class="mb-0 text-uppercase">salesman</h6>
            <hr />
            <div class="col-12">
                <a href="{{ url(ENV('ADMIN_FOLDER_NAME') . '/salesman/create') }}" class="btn btn-light px-5"
                    style="margin-bottom: 15px;">Add New</a>
            </div>
            <div class="card">
                <div class="card-body">
                    @if (session('status'))
                        <div class="alert alert-success">
                            {{ session('status') }}
                        </div>
                    @endif
                    @if (session('status-error'))
                        <div class="alert alert-danger">
                            {{ session('status-error') }}
                        </div>
                    @endif
                    <div class="table-responsive">
                        <table id="district" class="table table-striped table-bordered" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Name</th>
                                    <th>Address</th>
                                    <th>Email</th>
                                    <th>Mobilephone</th>
                                    <th>Branch</th>
                                    <th>Gender</th>
                                    <th>Active</th>
                                    <th>Last Updated At</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($salesmans as $s)
                                    <tr>
                                        <td>
                                            <a href="{{ url(ENV('ADMIN_FOLDER_NAME') . '/salesman/' . $s->id . '/edit') }}"
                                                style="text-decoration: underline;">{{ $s->name }}</a>
                                        </td>                                        
                                        <td>
                                            {!! $s->address .
                                                ', ' .
                                                ucwords(strtolower($s->subdistrict->sub_district_name)) .
                                                ', ' .
                                                $s->district->district_name .
                                                '<br/>' .
                                                $s->city->city_type .
                                                ' ' .
                                                $s->city->city_name .
                                                '<br/>' .
                                                $s->province->province_name .
                                                ' ' .
                                                $s->subdistrict->post_code !!}
                                        </td>
                                        <td>{{ $s->email }}</td>
                                        <td>{{ $s->mobilephone }}</td>
                                        <td>{{ $s->branch->name }}</td>
                                        <td>{{ $s->gender->title_ind }}</td>
                                        <td>{{ $s->active }}</td>
                                        <td>{{ date_format(date_create($s->updated_at), 'd M Y H:i:s') }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                            <tfoot>
                                <tr>
                                    <th>Name</th>
                                    <th>Address</th>
                                    <th>Email</th>
                                    <th>Mobilephone</th>
                                    <th>Branch</th>
                                    <th>Gender</th>
                                    <th>Active</th>
                                    <th>Last Updated At</th>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                    <hr />
                    <div class="input-group" style="margin-top: 15px;">
                        <a download="" href="{{ url(ENV('ADMIN_FOLDER_NAME') . '/salesman/salesman-export-xlsx') }}"
                            class="btn btn-light px-5" style="margin-bottom: 15px;">Export to Excel</a>
                    </div>
                    <form action="{{ url(ENV('ADMIN_FOLDER_NAME') . '/salesman/salesman-import') }}" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        <div class="input-group" style="margin-top: 15px;">
                            <input type="file" class="form-control @error('xlsx_file') is-invalid @enderror"
                                id="xlsx_file" name="xlsx_file" aria-describedby="inputGroupFileAddon04"
                                aria-label="Upload">
                            <button class="btn btn-light" type="submit" id="inputGroupFileAddon04">Import from
                                Excel</button>
                            @error('xlsx_file')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div>
    <!--end page wrapper -->
@endsection

@section('script')
    <script src="{{ asset('assets/plugins/datatable/js/jquery.dataTables.min.js') }}"></script>
    <script src="{{ asset('assets/plugins/datatable/js/dataTables.bootstrap5.min.js') }}"></script>
    <script>
        $(document).ready(function() {
            $('#district').DataTable();
        });
    </script>
@endsection
