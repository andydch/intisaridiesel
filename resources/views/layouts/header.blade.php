        ...
        <nav class="navbar navbar-expand">
            ...
            <div class="user-box dropdown">
                <a class="d-flex align-items-center nav-link dropdown-toggle dropdown-toggle-nocaret" href="#"
                    ...
                    @php
                        $profilePic = asset('upl/user-pics.jpg');
                        $user = \App\Models\Userdetail::with('user')
                        ->where([
                            'user_id' => Auth::user()->id,
                        ])
                        ->first();
                        if ($user) {
                            if (!is_null($user->profile_pic) && $user->profile_pic != '') {
                                $profilePic = asset('upl/employees/'.$user->profile_pic);
                            }
                        }
                    @endphp
                    <a href="{{ url('user-profile/update-profile/'.Auth::user()->slug) }}">
                        <img src="{{ $profilePic }}" class="user-img" alt="{{ Auth::user() ? Auth::user()->name : 'Guest' }}">
                    </a>
                    <div class="user-info ps-3">
                        <p class="user-name mb-0">{{ Auth::user() ? Auth::user()->name : 'Guest' }}</p>
                        <p class="designattion mb-0">
                            @if ((Auth::user() ? Auth::user()->id : 0) == 1)
                                {!! 'Administrator<br/>'.$user->branch->name !!}
                            @else
                                @if (!is_null($user))
                                    {{-- {{ $user->position }} --}}
                                    {{ !is_null($user->branch)?$user->branch->name:'' }}
                                @else
                                    @php
                                        $isVerified = \App\Models\User::where([
                                            'id' => Auth::user()->id,
                                        ])->first();
                                    @endphp
                                    @if (is_null($isVerified->email_verified_at))
                                        {{ 'not verified user' }}
                                    @else
                                        {{ 'not specified' }}
                                    @endif
