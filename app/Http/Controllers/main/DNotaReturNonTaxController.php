...
class DNotaReturNonTaxController extends Controller
{
    /**
    ...
    public function __invoke(Request $request)
    {
        ...
        try {
            ...
            for($i=0;$i<count($Ids);$i++){
                if($Ids[$i]!==''){
                    $del = Tx_nota_retur_non_tax::where('id','=',$Ids[$i])
                    ->where([
                        'approved_by' => null
                    ])
                    ->update([
                    ...
                    $delPart = Tx_nota_retur_part_non_tax::where('nota_retur_id','=',$Ids[$i])
                    ->update([
                        'active'=>'N',
                        'updated_by' => Auth::user()->id
                    ]);
