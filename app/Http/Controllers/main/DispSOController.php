...
class DispSOController extends Controller
{
    /**
    ...
    public function __invoke(Request $request)
    {
        ...
        ->select('tx_sales_orders.id','tx_sales_orders.sales_order_no AS order_no')
        ...
        ->when($request->sales_order_date!='#', function($query) use($request) {
            $query->whereRaw('DATE_FORMAT(tx_sales_orders.sales_order_date, "%d/%m/%Y")=\''.$request->sales_order_date.'\'');
        })
        ->where('tx_sales_orders.is_vat','=','Y')
