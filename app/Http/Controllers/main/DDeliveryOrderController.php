...
class DDeliveryOrderController extends Controller
{
    /**
    ...
    public function __invoke(Request $request)
    {
        ...
        try {
            ...
            for($i=0;$i<count($Ids);$i++){
                if($Ids[$i]!==''){
                    ...
                    $delPart = Tx_delivery_order_part::leftJoin('tx_delivery_orders AS tx_do','tx_delivery_order_parts.delivery_order_id','=','tx_do.id')
                    ...
                    ->where([
                        'tx_delivery_order_parts.delivery_order_id' => $Ids[$i],
                    ...
                    $del = Tx_delivery_order::where('id','=',$Ids[$i])
                    ->update([
                        'active'=>'N',
                        'updated_by' => Auth::user()->id
                    ]);
                    $delPart = Tx_delivery_order_part::where('delivery_order_id','=',$Ids[$i])
                    ->update([
                        'active'=>'N',
                        'updated_by' => Auth::user()->id
                    ]);

                    $q = Tx_delivery_order::where([
                        'id' => $Ids[$i],
                    ])
                    ->first();
                    if ($q){
        ...
        return redirect(ENV('TRANSACTION_FOLDER_NAME').'/faktur');
    }
}
